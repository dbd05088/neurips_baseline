        ...
        for i in indices:
            ...
            if transform is None:
                if not robust:
                    ...
                    #print("robust1!!")
                    if self.transform_on_gpu:
                        if type(robust_transform) == list:
                            origin_img = robust_transform[0](img)
                            image = robust_transform[1](img)
                            test_img = robust_transform[2](img)

                            images.append(image)
                            origin_images.append(origin_img)
                            test_images.append(test_img)
                        else:
                            images.append(robust_transform(img))
                        split_images.append(robust_test_transform(img))
                    else:
                        images.append(robust_transform(self.images[i]))
                        split_images.append(robust_test_transform(self.images[i]))
            else:
                if not robust:
                    if self.transform_on_gpu:
                    ...
                    if self.transform_on_gpu:
                        if type(robust_transform) == list:
                            origin_img = robust_transform[0](self.images[i])
                            image = robust_transform[1](self.images[i])
                            test_img = robust_transform[2](self.images[i])

                            images.append(image)
                            origin_images.append(origin_img)
                            test_images.append(test_img)
                        else:
                            images.append(robust_transform(self.images[i].to(self.device)))
                        split_images.append(robust_test_transform(self.images[i].to(self.device)))
                    else:
                        images.append(robust_transform(self.images[i]))
                        split_images.append(robust_test_transform(self.images[i]))
            labels.append(self.labels[i])
            split_labels.append(self.labels[i])
            self.cls_train_cnt[self.labels[i]] += 1
        data['image'] = torch.stack(images)
        data['label'] = torch.LongTensor(labels)

        if robust:
            split_data['image'] = torch.stack(split_images)
            split_data['label'] = torch.LongTensor(split_labels)
            if type(robust_transform) == list:
                data['origin_img'] = torch.stack(origin_images)
                data['test_img'] = torch.stack(test_images)

        if self.keep_history:
            self.previous_idx = np.append(self.previous_idx, indices)
        
        if not robust:
            return data, list(np.array(self.datalist)[indices])
        else:
            return data, split_data, list(np.array(self.datalist)[indices])
